...
//            .ToeicJukugoBeginner, .ToeicJukugoIntermediate, .ToeicJukugoAdvanced,
            .BunpouBeginner, .BunpouIntermediate, .BunpouAdvanced,
                .BunpouJunBeginner, .BunpouJunIntermediate, .BunpouJunAdvanced, 
//                .BunpouJunAdvanced, .JunBunpouBeginner
        ]
        // 除外したいケースをここで除外
    }
}
...
struct PentagonBunpouGraphLabelView: View {
    var label: String
    var index: Int
...
struct PentagonBunpouGraphBackgroundShape: Shape {
    func path(in rect: CGRect) -> Path {
        let center = CGPoint(x: rect.midX, y: rect.midY)
...
struct PentagonBunpouGraphView: View {
    ...
    var body: some View {
            GeometryReader { geometry in
                ...
                ZStack {
                    PentagonBunpouGraphBackgroundShape()
                    ...
                    ForEach(Array(QuizBunpouLevel.allCases.enumerated()), id: \.offset) { (i, _) in
                        PentagonBunpouGraphLabelView(label: labels[i], index: i)
                    }
                    // 目盛りの数字を表示する
...
struct PentagonBunpouView: View {
    ...
    var body: some View {
        VStack{
            PentagonBunpouGraphView(userId: authManager.currentUserId!, labels: ["資産計画グラフ1", "リスク管理グラフ1", "金融資産運用グラフ1", "タックスプランニンググラフ1","不動産グラフ1","相続・事業継承グラフ1"])
                .padding(.top,30)
            VStack(spacing: 0) {
                HStack{
