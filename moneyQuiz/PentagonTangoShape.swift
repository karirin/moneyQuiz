...
//            .incorrectTangoAnswer, .incorrectJukugoAnswer, .incorrectBunpouAnswer,
            ...
            .ToeicTangoBeginner, 
//                .ToeicTangoIntermediate, .ToeicTangoAdvanced,
...
struct PentagonTangoGraphShape: Shape {
...
struct PentagonTangoGraphLabelView: View {
    var label: String
    var index: Int
    ...
    var body: some View {
        GeometryReader { geometry in
            let angle = (2 * CGFloat.pi) / CGFloat(QuizTangoLevel.allCases.count) * CGFloat(index) - .pi / 2
...
struct PentagonTangoGraphBackgroundShape: Shape {
    func path(in rect: CGRect) -> Path {
        let center = CGPoint(x: rect.midX, y: rect.midY)
...
struct PentagonTangoGraphView: View {
    @State private var quizData = [QuizTangoLevel: QuizData]()
    let userId: String
    let labels: [String]
    ...
    var body: some View {
            GeometryReader { geometry in
                ...
                ZStack {
                    PentagonTangoGraphBackgroundShape()
                                    .stroke(Color.gray, lineWidth: 1)
                    PentagonTangoGraphShape(quizData: quizData)
                    ...
                    ForEach(Array(QuizTangoLevel.allCases.enumerated()), id: \.offset) { (i, _) in
                        PentagonTangoGraphLabelView(label: labels[i], index: i)
                    }
                    // 目盛りの数字を表示する
                ...
                .onAppear {
                    RateManager.shared.fetchTangoQuizData(userId: userId) { data in
                        self.quizData = data
                    }
                }
            }
...
struct PentagonTangoView: View {
    ...
    var body: some View {
        VStack{
            PentagonTangoGraphView(userId: authManager.currentUserId!, labels: ["資産計画グラフ3", "リスク管理グラフ3", "金融資産運用グラフ3", "タックスプランニンググラフ3","不動産グラフ3","相続・事業継承グラフ3"])
                .padding(.top,30)
            VStack(spacing: 0) {
                HStack{
                ...
                .padding()
                ScrollView{
                    ForEach(QuizTangoLevel.allCases, id: \.self) { level in
...
//        .background(Color(hue: 0.141, saturation: 0.058, brightness: 0.954))
        ...
        .onAppear {
            ...
            RateManager.shared.fetchTangoQuizData(userId: authManager.currentUserId!) { data in
                self.quizData = data
...
struct PentagonTangoShape_Previews: PreviewProvider {
