...
struct QuizIncorrectJukugoAnswerListView: View {
    ...
    var body: some View {
        VStack{
            if isDataLoaded {
                ...
                QuizView(quizzes: shuffledQuizList, quizLevel: .incorrectJukugoAnswer, authManager: authManager, audioManager: audioManager, isPresenting: $isPresenting, interstitial: sharedInterstitial)
            } else {
                ActivityIndicator()
            }
        }
        .onAppear {
            ...
            fetchIncorrectJukugoAnswers(userId: authManager.currentUserId!) { quizList in
...
func fetchIncorrectJukugoAnswers(userId: String, completion: @escaping ([QuizQuestion]) -> Void) {
...
struct QuizIncorrectJukugoAnswerListView_Previews: PreviewProvider {
    static var previews: some View {
        QuizIncorrectJukugoAnswerListView(isPresenting: .constant(false))
    }
}
