//
//  QuizIncorrectBunpouAnswerListView.swift
//  egQuiz
//
...
struct QuizIncorrectBunpouAnswerListView: View {
    ...
    var body: some View {
        VStack{
            if isDataLoaded {
                ...
                QuizView(quizzes: shuffledQuizList, quizLevel: .incorrectBunpouAnswer, authManager: authManager, audioManager: audioManager, isPresenting: $isPresenting, interstitial: sharedInterstitial)
            } else {
                ActivityIndicator()
            }
        }
        .onAppear {
            ...
            fetchIncorrectBunpouAnswers(userId: authManager.currentUserId!) { quizList in
...
func fetchIncorrectBunpouAnswers(userId: String, completion: @escaping ([QuizQuestion]) -> Void) {
...
struct QuizIncorrectBunpouAnswerListView_Previews: PreviewProvider {
    static var previews: some View {
        QuizIncorrectBunpouAnswerListView(isPresenting: .constant(false))
    }
}
